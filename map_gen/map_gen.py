...
from shapely.geometry import LineString, Point  # 引入点，线对象库
import math
from map_proto import map_lane_pb2
from map_proto import map_pb2
import sys

sys.path.append('/home/ab/VCodes/test_ws/hdmap_gen/map_proto')
...
def convert(p, p2, distance):
    ...
    # y/x求出夹角，夹角+pi/2是左转角，夹角-pi/2是右转角
...
with open(fpath, 'r') as f:
    for line in f:
        ...
        x = float(data[0])
        y = float(data[1])
        points.append((x, y))  # 读取txt文件中的x,y参数

path = LineString(points)  # 把上面的点转换成线
length = int(path.length)  # 求出线的长度

fmap = open("map_" + fpath.split("/")[-1] + ".txt", 'w')
id = 0
map = map_pb2.Map()  # 定义map的结构体，具体在map_pb2.proto中
road = map.road.add()  # 定义map中的road
road.id.id = "1"  # 定义并初始化road的id
section = road.section.add()  # 上同
section.id.id = "2"
lane = None
for i in range(length - 1):
    if i % 100 == 0:  # 每100m id加一
        id += 1
        # 通过create_lane创建lane中的一些属性：id等
